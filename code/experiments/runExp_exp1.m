...
%
% Manipulation is information (0:1:5)
%
% Initial 30 trials per condition: 180 total trials

function runExp_exp1
...
if ~exist([data_save subj.ID '_' subj.initials '/' subj.ID subj.initials '_settings.mat'],'file')
    ...
    % Creates trials
    [TR,subj] = assign_trials_exp1(subj,cfg);
...
% reveal a few interesting combinations of information/outcomes

if USE_training

    run_training_exp1(subj,cfg,windows,wB,machine,narrowing,...
...
summary_tex = sprintf(['Remember:\n\n'...
    ...
    '2. Consider INFORMATION when choosing to play or not']);
...
for tr = 1:length(TR)
    ...
    if strcmp(TR(tr).stake_decision,'accept')
        ...
        if isnan(TR(tr).stake_decision)
            ...
            Screen('FillRect', cfg.win_ptr, cfg.black);
            wait_string = 'Too slow! Please wait until next gamble is prepared.';
