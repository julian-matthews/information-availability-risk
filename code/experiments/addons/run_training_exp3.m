function run_training_exp3( subj,cfg,windows,wB,machine,narrowing,audio_samples,reveal_sound,reject_sound,device_num,USE_winnings )
...
% This version is designed for the 'early late' version of the task
...
% These are the faces that appear (ie, on top of non-info slots)
training.face_arrangement = [...
    1 -1 -1 1 1;... % Forced accept
    0 0 0 0 0;... % Forced reject
    1 1 -1 1 -1;... % demonstration of non-informative late reveal
    1 -1 -1 -1 1;... % demonstration of that non-informative are truly random also early reveal
...
for prac = 1:15
    ...
    % Random info_arrangement sorted early or late (50% probability)
    coin_toss = randi(2,1)-1;
    
    if coin_toss == 1
        TRAIN(prac).info_arrangement = sort(randi(2,1,5)-1,'ascend');
    else
        TRAIN(prac).info_arrangement = sort(randi(2,1,5)-1,'descend');
    end
